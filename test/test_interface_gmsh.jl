...
vertices2df =  CompScienceMeshes.gmsh_to_mesh_nodes(node_tags, node_coords, udim=2, vertextype=Float32)
...
elements = CompScienceMeshes.gmsh_get_elements(element_types, element_tags, element_node_tags; element=:triangle, order=1)
...
mesh_old_interface = CompScienceMeshes.read_gmsh_mesh(joinpath(@__DIR__, "assets/cuboid_triangle.msh"), physical="Top")
mesh_new_interface = load_gmsh_mesh(joinpath(@__DIR__, "assets/cuboid_triangle.msh"), physical="Top")

@test mesh_new_interface.vertices == mesh_old_interface.vertices
@test mesh_new_interface.faces == mesh_old_interface.faces

# Ensure backwards compatibility: Tetrahedrons

mesh_old_interface = CompScienceMeshes.read_gmsh3d_mesh(joinpath(@__DIR__, "assets/cuboid_tetrahedron.msh"))
mesh_new_interface = load_gmsh_mesh(joinpath(@__DIR__, "assets/cuboid_tetrahedron.msh"), element=:tetrahedron, sort=false)
...
mesh_new_interface = load_gmsh_mesh(joinpath(@__DIR__, "assets/cuboid_quadrangle.msh"), element=:quadrangle)
...
mesh_old_interface = CompScienceMeshes.read_gmsh3d_mesh(joinpath(@__DIR__, "assets/cuboid_tetrahedron.msh"),physical="Interior")
mesh_new_interface = load_gmsh_mesh(joinpath(@__DIR__, "assets/cuboid_tetrahedron.msh"), element=:tetrahedron, physical="Interior")
